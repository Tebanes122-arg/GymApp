...
export default function Login() {
  ...
  // si llega desde un enlace de invitación del gimnasio (?inv=DNI&n=Nombre&a=Apellido)
  useEffect(() => {
    if (!router.isReady) return;
    const { inv, n, a, tel } = router.query;
    if (inv) {
      setModo('registro');
      setForm((f) => ({
        ...f,
        dni: String(inv),
        nombre: n ? String(n) : f.nombre,
        apellido: a ? String(a) : f.apellido,
        telefono: tel ? String(tel) : f.telefono,
      }));
      setEsError(false);
      setMsg('¡Te invitó GymApp! 💛 Completá tu email y una contraseña para activar tu cuenta.');
    }
