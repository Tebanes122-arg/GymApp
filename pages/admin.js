  ...
  useEffect(() => {
    async function cargar() {
      try {
        ...
        if (yo?.rol !== 'dueno' && yo?.rol !== 'admin') {
          setSinPermiso(true);
          setCargando(false);
          return;
        }
        setMiNombre(yo?.nombre || '');
        setMiRol(yo?.rol || '');
      ...
      <div className="shell">
        ...
        <div className="seccion" style={{ textAlign: 'center', paddingTop: 40 }}>
          ...
          <p className="subtitulo" style={{ maxWidth: 420, margin: '14px auto 0' }}>
            Este panel es exclusivo de la administración de GymApp.
    ...
    <div className="shell">
      <div className="topbar">
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
          <span className="rol-chip">{miRol === 'admin' ? (miNombre || 'Esteban Fernández') : 'Panel del dueño'}</span>
          <button className="btn-salir" onClick={async () => { await cerrarSesion(); router.push('/login'); }}>Salir</button>
        </div>
      </div>
      ...
      <div>
        ...
        <h1 className="titulo">{miRol === 'admin' ? `Hola, ${miNombre || 'Esteban'}` : 'Hola, GymApp'}</h1>
      ...
      <div className="seccion" style={{ marginTop: 18, display: 'flex', gap: 10, flexWrap: 'wrap' }}>
        ...
        <Link href="/asistencias" className="btn btn-secundario" style={{ flex: 1, minWidth: 200 }}>
          📋 Asistencias y horas pico
        </Link>
      </div>
              ...
              <div className="fila" key={a.id}>
                ...
                {a.telefono && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    href={linkWsp(a.telefono, `Hola ${a.nombre}! Te escribimos de GymApp 💪 Tu cuota está vencida, te esperamos para regularizarla y que sigas entrenando con nosotros.`)}
                  >
              ...
              <div className="fila" key={a.id}>
                ...
                {a.telefono && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    href={linkWsp(a.telefono, `Hola ${a.nombre}! Te recordamos de GymApp que tu cuota vence pronto 💪 ¡Te esperamos!`)}
                  >
                    Recordar por WhatsApp
                  </a>
                )}
              ...
              <div className="fila" key={a.id}>
                ...
                {a.telefono && (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    href={linkWsp(a.telefono, `Hola ${a.nombre}! Te extrañamos en GymApp 💪 ¿Todo bien? Te esperamos para retomar el entrenamiento.`)}
                  >
                    Escribirle
                  </a>
                )}
