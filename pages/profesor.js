...
import { supabase, GIMNASIO_ID, diasHasta, usuarioActual, cerrarSesion } from '../lib/supabase';
...
export default function Profesor() {
  ...
  const [vista, setVista] = useState('plantillas'); // plantillas | editar | asignar | alumnos
  ...
  async function cargar() {
    ...
    const hoy = new Date();
    const offh = hoy.getTimezoneOffset();
    const hoyStr = new Date(hoy.getTime() - offh * 60000).toISOString().slice(0, 10);

    const [rCat, rPlant, rAl, rAsis] = await Promise.all([
      ...
      // el profe ve nombre, contacto y vencimiento (para el semáforo), NO montos ni ingresos
      supabase.from('alumnos').select('id, nombre, apellido, telefono, vencimiento, activo').eq('gimnasio_id', GIMNASIO_ID).eq('rol', 'alumno').eq('activo', true).order('apellido'),
      supabase.from('asistencias').select('alumno_id, hora').eq('gimnasio_id', GIMNASIO_ID).eq('fecha', hoyStr),
    ]);
    setCatalogo(rCat.data || []);
    setPlantillas(rPlant.data || []);
    setAlumnos(rAl.data || []);
    setAsistenciasHoy(rAsis.data || []);
    setEstado('ok');
  }
  ...
  return (
    <div className="shell">
      ...
      {(vista === 'plantillas' || vista === 'alumnos') && (
        <div className="login-tabs" style={{ marginTop: 18 }}>
          <button className={'login-tab' + (vista === 'plantillas' ? ' activo' : '')} onClick={() => setVista('plantillas')}>Rutinas</button>
          <button className={'login-tab' + (vista === 'alumnos' ? ' activo' : '')} onClick={() => setVista('alumnos')}>Alumnos</button>
        </div>
      )}
      ...
      {/* ============ ALUMNOS (vista del profesor) ============ */}
      {vista === 'alumnos' && (() => {
        const presentesHoy = new Set(asistenciasHoy.map((a) => a.alumno_id));
        const ordenados = [...alumnos].sort((x, y) => {
          const px = presentesHoy.has(x.id) ? 0 : 1;
          const py = presentesHoy.has(y.id) ? 0 : 1;
          return px - py;
        });
        return (
          <>
            <div>
              ...
              <p className="subtitulo">{presentesHoy.size} {presentesHoy.size === 1 ? 'presente' : 'presentes'} hoy · {alumnos.length} activos</p>
            </div>
            <div className="seccion">
              <div className="lista">
                {ordenados.map((a) => {
                  const dias = diasHasta(a.vencimiento);
                  const debe = dias === null || dias < 0;
                  const vino = presentesHoy.has(a.id);
                  return (
                    <div className="fila" key={a.id}>
                      <div className="avatar">{iniciales(a.nombre, a.apellido)}</div>
                      <div className="fila-info">
                        <div className="fila-nombre">
                          {a.nombre} {a.apellido} {vino && <span className="tiene-cuenta">✓ vino hoy</span>}
                        </div>
                        <div className="fila-detalle">
                          {a.telefono ? a.telefono : 'Sin teléfono'}
                          {debe && <span style={{ color: 'var(--rojo)', fontWeight: 700 }}> · Debe cuota</span>}
                        </div>
                      </div>
                      {debe && (
                        <span className="estado estado-vencida" title="Avisale al dueño cuando abone">Avisar al dueño</span>
                      )}
                    </div>
                  );
                })}
              </div>
              <p className="nota-privacidad" style={{ marginTop: 16 }}>
                Cuando un alumno con cuota vencida abone, avisale al dueño para que lo reactive. Los pagos y montos los maneja solo el dueño.
              </p>
            </div>
          </>
        );
