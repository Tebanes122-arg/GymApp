...
const FORM_VACIO = { nombre: '', apellido: '', dni: '', telefono: '', email: '', instagram: '', plan_id: '' };
...
function linkWspInvitacion(a) {
  ...
  const msg = 'Hola ' + a.nombre + '! Te invitamos a activar tu cuenta en la app de GymApp. Con este link ya quedan cargados tus datos, solo pones tu email y una contrasena:\n\n' + url + '\n\nAhi vas a ver tu rutina, tus pesos y el estado de tu cuota. Te esperamos en GymApp!';
...
export default function Alumnos() {
  ...
  async function cargar() {
    ...
    const [rAl, rPl] = await Promise.all([
      supabase
        .from('alumnos')
        .select('id, nombre, apellido, dni, telefono, email, instagram, vencimiento, activo, rol, plan_id, auth_user_id, planes(nombre, precio, precio_transferencia)')
  ...
  function abrirEditar(a) {
    setForm({
      ...
      telefono: a.telefono || '', email: a.email || '', instagram: a.instagram || '', plan_id: a.plan_id || '',
    });
  ...
  async function registrarPago() {
    ...
    // nuevo vencimiento: 30 días desde la fecha de pago elegida,
    // o desde el vencimiento actual si el alumno todavía estaba al día
    const fechaPago = new Date(pagoFecha + 'T12:00:00');
    const venceActual = a.vencimiento ? new Date(a.vencimiento + 'T12:00:00') : null;
    const base = venceActual && venceActual > fechaPago ? venceActual : fechaPago;
              ...
              <div className="fila" key={a.id}>
                ...
                <div className="fila-info">
                  ...
                  <div className="fila-detalle">
                    {a.planes?.nombre || 'Sin plan'} · Vence {fechaLinda(a.vencimiento)} · DNI {a.dni || '—'}{a.instagram && <> · <a href={`https://instagram.com/${a.instagram}`} target="_blank" rel="noreferrer" style={{ color: 'var(--fuego)' }}>@{a.instagram}</a></>} {a.auth_user_id && <span className="tiene-cuenta">✓ con cuenta</span>}
                  </div>
                </div>
          ...
          <div className="modal-ficha" onClick={(e) => e.stopPropagation()}>
            ...
            {(modal.tipo === 'alta' || modal.tipo === 'editar') && (
              <>
                ...
                <div className="login-form" style={{ marginTop: 16 }}>
                  ...
                  <input className="input-login" placeholder="Instagram (opcional, ej: juanperez)" value={form.instagram} onChange={(e) => setForm({ ...form, instagram: e.target.value })} />
            ...
            {modal.tipo === 'pago' && (
              <>
                ...
                <div className="login-form" style={{ marginTop: 16 }}>
                  <label className="ficha-sub" style={{ marginTop: 0 }}>Fecha del pago</label>
                  <input className="input-login" type="date" value={pagoFecha} onChange={(e) => setPagoFecha(e.target.value)} />
                  <label className="ficha-sub">Monto</label>
                  ...
                  <p className="nota-privacidad" style={{ textAlign: 'left' }}>
                    {(() => {
                      const fp = new Date(pagoFecha + 'T12:00:00');
                      const va = modal.alumno.vencimiento ? new Date(modal.alumno.vencimiento + 'T12:00:00') : null;
                      const base = va && va > fp ? va : fp;
                      const venc = new Date(base.getTime() + 30 * 86400000);
                      const txt = venc.toLocaleDateString('es-AR', { day: '2-digit', month: '2-digit', year: 'numeric' });
                      return `Nuevo vencimiento: ${txt} (30 días ${va && va > fp ? 'desde el vencimiento actual' : 'desde la fecha del pago'}).`;
                    })()}
                  </p>
