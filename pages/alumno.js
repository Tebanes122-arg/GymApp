...
// Ubicación de GymApp (Garupá, colectora RN12) y radio permitido
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        if (d <= RADIO_METROS) {
          ...
          setMarcando(false);
          setMsgPresente(
            `Estás a ${d >= 1000 ? (d / 1000).toFixed(1) + ' km' : d + ' m'} de GymApp. El presente solo se puede marcar desde el gimnasio 😉`
          );
        }
      },
      (err) => {
      ...
      <div className="shell">
        ...
        <div className="seccion" style={{ textAlign: 'center', paddingTop: 40 }}>
          ...
          <p className="subtitulo" style={{ maxWidth: 420, margin: '14px auto 0' }}>
            ...
            Apenas GymApp la apruebe vas a poder ver tu rutina, tu cuota y tu progreso.
          </p>
        </div>
      </div>
    );
      ...
      <div className="seccion">
        ...
        {!presenteHoy && (
          <p className="nota-privacidad">
            Tu ubicación se usa únicamente para verificar que estés en GymApp al marcar el presente. No se almacena.
          </p>
        )}
      </div>
      )}
